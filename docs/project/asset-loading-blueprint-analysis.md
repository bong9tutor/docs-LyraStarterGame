...
확인일: 2026-05-25 
분석 도구: Monolith MCP (`blueprint_query.get_cdo_properties` 검증 완료) + Rider MCP (`search_in_files_by_regex` 검증 완료) + ini 파일 직접 확인 
...
(핵심) **핵심 정정**: 라이라의 4개 설정 절은 `DefaultEngine.ini` 가 아니라 **`DefaultGame.ini`** 에 있음. `DefaultEngine.ini` 에는 `AssetManagerClassName` 과 `GlobalDefaultGameMode` 만.

### `Config/DefaultEngine.ini` - 2개 핵심 키 (라이라 검증 검증 완료)
...
### `Config/DefaultGame.ini` - 4개 절 (라이라 검증 검증 완료)
...
## DefaultGameData 자산 - CDO 검증 검증 완료
...
## Experience 자산의 AssetBundleData - CDO 검증 검증 완료
...
**의미**:
- **Runtime 측 (검증 완료 verified)**: `LAS_ShooterGame_StandardHUD` 의 12 widget 이 `Client` bundle 에 등록 → dedicated server 의 `StartExperienceLoad()` 는 `LoadStateClient` 를 `BundlesToLoad` 에 포함하지 않으므로 server 는 widget 자산을 **runtime 메모리에 로드하지 않음**.
...
(핵심) **정정**: 이전 "9개" 는 `Source/LyraGame/` 한정 검색 결과. GameFeature 플러그인 런타임 코드까지 포함하면 **10곳**.

### 라이라 코어 (`Source/LyraGame/`) - 9곳 검증 완료
...
### GameFeature 플러그인 (`Plugins/GameFeatures/`) - 1곳 추가 검증 완료
...
## GameFeature 플러그인 5개 - 메타데이터 검증 완료
...
| Experience BP (ShooterCore 플러그인) | `/ShooterCore/Experiences/B_*` | **3개** - `B_LyraShooterGame_ControlPoints` · `B_ShooterGame_Elimination` · `B_ShooterGame_Perf` (검증 검증 완료) |
