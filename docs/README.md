...
|-----------|------|------------------------|
| [`common/`](common/) | **컨텐츠 생성 정책 (A)** — 분석·학습 문서를 어떻게 만들고 표현할지 (도구 사용·작업 절차·HTML 사양) | ✅ 폴더 통째로 카피 가능 |
| [`project/`](project/) | **프로젝트 컨텍스트 + 시스템별 검증 원장** — 라이라 아키텍처 산문·검증 표 + 시스템 분석 결과 (HTML 의 단일 출처) | ❌ 라이라 종속 |
| [`../tools/`](../tools/) — `check-doc-links.cjs` · `wrap-tables.cjs` | **공통 보조 도구** — `common/` 정책이 안내하는 정적 검사·일괄 보정 스크립트 | ✅ `common/` 와 함께 카피 (선택 — `tools/generate-dynamic-html.cjs` 는 legacy 라 제외) |
