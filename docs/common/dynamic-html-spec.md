...
- `✅` → `✓` (사양 정의 글리프). 배지 클래스 안에 두면 자동으로 녹색 톤. 본문 인벤토리에서도 `✓` 만 사용.
- `①` `②` ... `⑦` → 카드·블록의 번호는 (1) 카드 제목 자체에 번호를 두지 않고 카드 자체로 시각 분리, 또는 (2) `<span class="struct-num">1</span>` 같은 별도 라벨로 (이 라벨은 CSS 로 시각화). enclosed 숫자 글리프 직접 본문 삽입 금지.
- `★` → 강조는 `<strong>` 태그 또는 `note-info` 박스로. 별 모양 글리프 본문 삽입 금지.
